...
import { StatutFacture, ModePaiement } from '../models/facture';
 ...
 */
export async function getRDVDuJour(req: Request, res: Response) {
  try {
 ...
 */
export async function getRDVDeLaSemaine(req: Request, res: Response) {
  try {
    const date = req.query.date || new Date().toISOString().split('T')[0];
 ...
 */
export async function getRDVEmployeDuJour(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const date = req.query.date || new Date().toISOString().split('T')[0];
 ...
 */
export async function getDisponibilites(req: Request, res: Response) {
  try {
    ...
    for (let heure = 8; heure < 18; heure++) {
      for (let minute = 0; minute < 60; minute += 30) {
        ...
        const estPris = rdvExistants.some((rdv: any) => {
    ...
    await pool.query(
      `UPDATE Facture
       SET montantPaye = ?,
           ...
           updatedAt = NOW()
       WHERE idFacture = ?`,
      [nouveauMontantPaye, nouveauStatut, modePaiement, datePaiement || new Date(), idFacture]
    );
 ...
 */
export async function getHistoriquePatient(req: Request, res: Response) {
  try {
    const { id } = req.params;
 ...
 */
export async function getFacturesImpayeesPatient(req: Request, res: Response) {
  try {
    const { id } = req.params;
